...
import json, os, datetime, psycopg2
...
def add_lease():
    data = request.json; now = datetime.datetime.utcnow().isoformat()
    conn = get_db(); cur = conn.cursor()
    cur.execute(
        ...
        (data.get('id'), data.get('tenant'), data.get('address'), data.get('building'),
         data.get('expiration'), data.get('exp_sort'), data.get('source'), data.get('notes'), now))
...
def update_lease(lid):
    ...
    cur.execute(
        ...
        (data.get('tenant'), data.get('address'), data.get('building'), data.get('expiration'),
         data.get('exp_sort'), data.get('source'), data.get('notes'), now, lid))
    conn.commit(); cur.close(); conn.close()
    return jsonify({'ok': True})
